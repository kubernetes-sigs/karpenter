...
// ResourceSliceController manages ResourceSlice lifecycle based on periodic polling of nodes and DRADriverConfig CRD
type ResourceSliceController struct {
	kubeClient client.Client
	driverName string
	namespace  string // Namespace where DRADriverConfig CRD is located
}
...
func (r *ResourceSliceController) reconcileAllNodes(ctx context.Context) error {
	...
	// Get THE single DRADriverConfig CRD for this driver
	// CRD name must match driver name (Design A: one CRD per driver)
	draConfig := &v1alpha1.DRADriverConfig{}
	err := r.kubeClient.Get(ctx, types.NamespacedName{
		Name:      r.driverName, // CRD name = driver name
		Namespace: r.namespace,
	}, draConfig)

	if errors.IsNotFound(err) {
		logger.V(1).Info("no DRADriverConfig found for driver, cleaning up all ResourceSlices", "driver", r.driverName)
		// No config exists - cleanup all ResourceSlices
		return r.cleanupOrphanedResourceSlices(ctx)
	}

	if err != nil {
		return serrors.Wrap(fmt.Errorf("getting DRADriverConfig, %w", err), "DRADriverConfig", klog.KRef(r.namespace, r.driverName))
	}

	// Verify driver name matches (sanity check)
	if draConfig.Spec.Driver != r.driverName {
		logger.Error(fmt.Errorf("driver name mismatch"), "DRADriverConfig driver field doesn't match controller driver",
	...
	// Convert CRD to internal config format
	cfg := r.convertToInternalConfig(draConfig)
...
// convertToInternalConfig converts DRADriverConfig CRD to internal config format
func (r *ResourceSliceController) convertToInternalConfig(draConfig *v1alpha1.DRADriverConfig) *config.Config {
	internalMappings := make([]config.Mapping, len(draConfig.Spec.Mappings))

	for i, crdMapping := range draConfig.Spec.Mappings {
		internalMappings[i] = config.Mapping{
			Name:              crdMapping.Name,
			NodeSelectorTerms: crdMapping.NodeSelectorTerms,
			ResourceSlice: resourcev1.ResourceSliceSpec{
				Driver:       draConfig.Spec.Driver,
				Pool:         crdMapping.ResourceSlice.Pool,
				Devices:      crdMapping.ResourceSlice.Devices,
				NodeName:     nil, // Filled in later per-node
				AllNodes:     crdMapping.ResourceSlice.AllNodes,
				NodeSelector: crdMapping.ResourceSlice.NodeSelector,
			},
		}
	}
...
func (r *ResourceSliceController) reconcileNodeResourceSlices(ctx context.Context, node *corev1.Node, cfg *config.Config) ([]string, error) {
	...
	for _, mapping := range matchingMappings {
		// ResourceSlice naming: karpenter-sh-<nodename>-<mapping-name>
		// Sanitize driver name: "karpenter.sh" -> "karpenter-sh"
...
func (r *ResourceSliceController) resourceSliceNeedsUpdate(existing *resourcev1.ResourceSlice, desired *resourcev1.ResourceSliceSpec) bool {
	...
	// For simplicity, we can do a deep comparison of devices
	// In production, might want more sophisticated comparison
	// For now, consider it needing update if device count differs or pool differs
...
// Example: "karpenter.sh" -> "karpenter-sh"
