...
// DRADriverConfigSpec defines the desired state of DRADriverConfig
type DRADriverConfigSpec struct {
	// driver specifies the DRA driver name that will manage these resources.
	// For single-driver support (Phase 1), this must be "karpenter.sh".
	// Future: Will support simulating other drivers like "gpu.nvidia.com".
	//
	// +required
	// +kubebuilder:validation:Enum=karpenter.sh
...
type Mapping struct {
	...
	// Used in ResourceSlice naming: karpenter-sh-<nodename>-<mapping-name>
	//
	// +required
	...
	// - spec.driver (from DRADriverConfigSpec.Driver)
	//
	// +required
...
// - driver (filled from DRADriverConfigSpec.Driver)
...
// DRADriverConfigStatus defines the observed state of DRADriverConfig
type DRADriverConfigStatus struct {
...
// DRADriverConfig is the Schema for the dradriverconfigs API.
// It configures how the KWOK DRA driver creates ResourceSlices for KWOK nodes.
type DRADriverConfig struct {
	metav1.TypeMeta `json:",inline"`
	// metadata contains the standard object metadata.
	// +optional
	metav1.ObjectMeta `json:"metadata,omitempty"`

	// spec defines the desired state of DRADriverConfig.
	// +required
	Spec DRADriverConfigSpec `json:"spec,omitzero,omitempty"`
	// status defines the observed state of DRADriverConfig.
	// +optional
	Status *DRADriverConfigStatus `json:"status,omitempty"`
}

// +kubebuilder:object:root=true

// DRADriverConfigList contains a list of DRADriverConfig
type DRADriverConfigList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata,omitempty"`
	Items           []DRADriverConfig `json:"items"`
}
...
func init() {
	SchemeBuilder.Register(&DRADriverConfig{}, &DRADriverConfigList{})
}
