...
func main() {
	...
	logger.Info("Starting DRA KWOK Driver (Design A: one CRD per driver)")
	...
	// Design A: CRD name must match driver name (karpenter.sh)
	resourceSliceController := controllers.NewResourceSliceController(
		mgr.GetClient(),
		"karpenter.sh", // Driver name (also used as CRD name)
		"karpenter",    // Namespace where DRADriverConfig CRD is located
	)
