...
For non-feature contributions, open a pull request referencing the relevant issue. If no issue exists, create one first for bug fixes so the problem can be discussed. For small refactors, test improvements, or documentation changes, a PR without a prior issue is fine.
