...
var _ = Describe("Performance", Label(debug.NoWatch), func() {
	Context("Host Name Spreading Deployment Reg", func() {
		It("should efficiently scale two deployments with host name topology spreading", func() {
			...
			Expect(scaleOutReport.KarpenterAvgCPUCores).To(BeNumerically("<", 1.25+CPUOverheadCores()),
				"Karpenter controller avg CPU should be less than 1.25 cores during scale-out")
			...
			Expect(consolidationReport.KarpenterP95MemoryMB).To(BeNumerically("<", 660+MemoryOverheadMB()),
				"Karpenter controller P95 memory should be less than 660 MB during consolidation")
			Expect(consolidationReport.KarpenterAvgCPUCores).To(BeNumerically("<", 1.05+CPUOverheadCores()),
				"Karpenter controller avg CPU should be less than 1.05 cores during consolidation")
