...
var _ = Describe("Performance", Label(debug.NoWatch), func() {
	Context("Self Anti-Affinity Deployment Interference", func() {
		It("should efficiently scale two deployments with self anti-affinity", func() {
			...
			// TODO: Re-enable once thresholds are calibrated for process_resident_memory_bytes
			// Expect(scaleOutReport.KarpenterP95MemoryMB).To(BeNumerically("<", 300+MemoryOverheadMB()),
			// 	"Karpenter controller P95 memory should be less than 300 MB during scale-out")
			// Expect(scaleOutReport.KarpenterP95CPUCores).To(BeNumerically("<", 1.10+CPUOverheadCores()),
			// 	"Karpenter controller P95 CPU should be less than 1.10 cores during scale-out")
			...
			Expect(interferenceReport.TotalReservedMemoryUtil).To(BeNumerically(">", 0.40),
				"Average memory utilization should be greater than 40%")
			// TODO: Re-enable once thresholds are calibrated for process_resident_memory_bytes
			// Expect(interferenceReport.KarpenterP95MemoryMB).To(BeNumerically("<", 550+MemoryOverheadMB()),
			// 	"Karpenter controller P95 memory should be less than 550 MB during scale-out")
			// Expect(interferenceReport.KarpenterP95CPUCores).To(BeNumerically("<", 1.0+CPUOverheadCores()),
			// 	"Karpenter controller P95 CPU should be less than 1.0 cores during scale-out")
			...
			// TODO: Re-enable once thresholds are calibrated for process_resident_memory_bytes
			// Expect(consolidationReport.KarpenterP95MemoryMB).To(BeNumerically("<", 450+MemoryOverheadMB()),
			// 	"Karpenter controller P95 memory should be less than 450 MB during consolidation")
			// Expect(consolidationReport.KarpenterP95CPUCores).To(BeNumerically("<", 0.80+CPUOverheadCores()),
			// 	"Karpenter controller P95 CPU should be less than 0.80 cores during consolidation")
