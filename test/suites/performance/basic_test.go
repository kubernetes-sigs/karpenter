...
var _ = Describe("Performance", Label(debug.NoWatch), func() {
	Context("Basic Deployment", func() {
		It("should efficiently scale two deployments with different resource profiles", func() {
			...
			Expect(scaleOutReport.KarpenterMemoryMB).To(BeNumerically("<", 5000),
				"Karpenter controller memory should be less than 5000 MB during scale-out")
			...
			Expect(consolidationReport.KarpenterMemoryMB).To(BeNumerically("<", 20000),
				"Karpenter controller memory should be less than 20000 MB during consolidation")
