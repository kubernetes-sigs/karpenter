...
var _ = Describe("Expiration", func() {
	...
	Context("Budgets", func() {
		...
		It("should not continue to disrupt nodes that have been the target of pod nomination", func() {
			...
			nodePool.Spec.Disruption.ExpireAfter = v1beta1.NillableDuration{}
			...
			for _, p := range pods {
				...
				delete(p.Annotations, v1beta1.DoNotDisruptAnnotationKey)
				env.ExpectUpdated(p)
			}
		...
		It("should respect budgets for non-empty delete expiration", func() {
			...
			for _, pod := range pods {
				delete(pod.Annotations, v1beta1.DoNotDisruptAnnotationKey)
		...
		It("should respect budgets for non-empty replace expiration", func() {
			...
			By("enabling expiration")
			nodePool.Spec.Disruption.ExpireAfter = v1beta1.NillableDuration{Duration: lo.ToPtr(30 * time.Second)}
			env.ExpectUpdated(nodePool)
	...
	Context("Failure", func() {
		It("should not continue to expire if a node never registers", func() {
			...
			Consistently(func(g Gomega) {
				...
				startingNodeClaimUIDs := lo.Map(startingNodeClaimState, func(nc *v1beta1.NodeClaim, _ int) types.UID { return nc.UID })
				nodeClaimUIDs := lo.Map(nodeClaims.Items, func(nc v1beta1.NodeClaim, _ int) types.UID { return nc.UID })
