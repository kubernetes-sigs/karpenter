...
var _ = Describe("DRA KWOK Driver", func() {
	var (
		draDriverConfig *drav1alpha1.DRADriverConfig
	...
	Context("GPU Configuration", func() {
		BeforeEach(func() {
			// Create DRADriverConfig CRD with GPU configuration
			// Design A: CRD name must match driver name
			draDriverConfig = &drav1alpha1.DRADriverConfig{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "karpenter.sh",
					Namespace: "karpenter",
				},
				Spec: drav1alpha1.DRADriverConfigSpec{
					Driver: "karpenter.sh",
		...
		It("should create and manage ResourceSlices based on CRD configuration", func() {
			By("Creating the DRADriverConfig CRD")
		...
		It("should handle DRADriverConfig updates dynamically", func() {
			By("Creating initial DRADriverConfig")
			env.ExpectCreated(draDriverConfig)
			...
			By("Updating the DRADriverConfig with new device configuration")
			Eventually(func() error {
				// Fetch latest version to avoid conflicts
				latest := &drav1alpha1.DRADriverConfig{}
	...
	Context("Configuration Validation", func() {
		It("should reject invalid DRADriverConfig via API server validation", func() {
			// With CRD, invalid configs are rejected at creation time by API server
			// Design A: CRD name must match driver name
			invalidConfig := &drav1alpha1.DRADriverConfig{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "karpenter.sh",
					Namespace: "karpenter",
				},
			...
			By("Attempting to create invalid DRADriverConfig")
			// API server should reject this due to validation rules
			err := env.Client.Create(env.Context, invalidConfig)
			Expect(err).To(HaveOccurred(), "API server should reject invalid DRADriverConfig")
			...
			invalidConfig2 := &drav1alpha1.DRADriverConfig{
				ObjectMeta: metav1.ObjectMeta{
	...
	Context("DRA Pod Scheduling in KWOK Environment", func() {
		BeforeEach(func() {
			// Create DRADriverConfig with device configuration
			// Design A: CRD name must match driver name
			draDriverConfig = &drav1alpha1.DRADriverConfig{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "karpenter.sh",
		...
		It("should ignore DRA pods when IgnoreDRARequests is enabled (default behavior)", func() {
			By("Creating the DRADriverConfig to simulate DRA infrastructure")
			env.ExpectCreated(draDriverConfig)
		...
		It("should prepare DRA testing infrastructure for future Karpenter DRA implementation", func() {
			By("Creating the DRADriverConfig to set up DRA KWOK driver")
			env.ExpectCreated(draDriverConfig)
	...
	Context("CRD-Specific Features", func() {
		// Test removed: "should merge multiple DRADriverConfigs with same driver"
		// Reason: Incompatible with Design A (One CRD Per Driver architecture)
		// Design A requires exactly ONE CRD named "karpenter.sh" per driver - no merging

		It("should support DRADriverConfig CRD status fields", func() {
			By("Creating DRADriverConfig")
			// Design A: CRD name must match driver name
			draDriverConfig = &drav1alpha1.DRADriverConfig{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "karpenter.sh",
			...
			retrieved := &drav1alpha1.DRADriverConfig{}
			Eventually(func() error {
				return env.Client.Get(env.Context, client.ObjectKeyFromObject(draDriverConfig), retrieved)
			}, 10*time.Second, 1*time.Second).Should(Succeed(), "DRADriverConfig should be readable")

			By("Verifying status subresource exists and can be updated")
			// The status fields should exist as zero values even if not yet populated by controller
			// Note: Conditions starts as nil, not empty slice
			Expect(retrieved.Status.ResourceSliceCount).To(Equal(int32(0))) // Not yet populated
			Expect(retrieved.Status.NodeCount).To(Equal(int32(0)))          // Not yet populated
				...
				latest.Status.Conditions = []metav1.Condition{
					{
						...
						Message:            "DRADriverConfig is valid and ready",
					},
				}
			...
			updated := &drav1alpha1.DRADriverConfig{}
