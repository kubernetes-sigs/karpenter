...
7. Karpenter adds the official ["out-of-service" taint](https://kubernetes.io/docs/concepts/architecture/nodes/#non-graceful-node-shutdown), instructing Kubernetes to forcefully remove all remaining pods. This requires K8s 1.26 to be available without setting a feature gate.
8. Any remaining pods are forcibly drained, ignoring eviction (bypassing PDBs, preStop hooks, pod terminationGracePeriod, etc).
9. The standard Karpenter reconciliation loop for the termination controller succeeds, and the node is formally removed from the cloud provider.
...
2. Enabling this feature can result in termination of user pods in violation of their configured grace periods or PDBs.
3. Because of the optimization stated above, user pods could be terminated in violdation of grace periods and PDBs **before** the configured NodeClaim grace period is reached.
