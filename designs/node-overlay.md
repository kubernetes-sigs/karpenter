...
- A percentage of the original price (e.g., 10% for reduction, 115% for increase)
...
spec:
  ...
  priceAdjustment: 500  # Set price to $5.00 of original
  # or
  priceAdjustment: -750  # Set price to -$7.50 of original
  # or
  priceAdjustment: "10%" # Set price to 10% of original
  # or
  priceAdjustment: "115%" # Set price to 115% of original
```
...
Instead of creating a new NodeOverlay CRD, an alternative approach would be to extend the existing NodePool API to include price adjustment and capacity modification capabilities. This section explores this alternative and explains why it was not chosen as the recommended solution.
...
```
> karpenter-overlay diagnose --overlay price-modification extended-resource --nodepool default
...
The NodeOverlay CRD will be introduced as a v1alpha1 feature, gated behind a feature flag in Karpenter. While the initial proposal focused solely on the priceAdjustment field, emerging users use cases, such as HugePage resource management, demonstrate a clear need for capacity field support. Given the validated users requirements, both priceAdjustment and capacity fields will be included in the first API iteration. The graduation path for NodeOverlay will align with Karpenter's established CRD maturation process. Rather than adhering to a fixed timeline, the progression to stable status will be driven by community adoption and feedback, primarily measured through GitHub Analytics via issue discussions, feature-related PRs, and community engagement patterns. This data-driven approach ensures the API meets real-world requirements and use patterns before stabilization.
