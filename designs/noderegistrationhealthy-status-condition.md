...
We propose adding new metrics to track how long it takes pods to become bound and ready, but only for pods scheduled to NodePools that have successfully registered at least one node (indicated by `NodeRegistrationHealthy: True`).

This approach means:
- If a NodePool has network connectivity issues (like NACL blocks) preventing any nodes from registering, no metrics will be collected for pods scheduled to that NodePool
- Metric collection for a NodePool begins only after its first successful node registration. Once `NodeRegistrationHealthy: True`, and a node is launched with a bad AMI then we will still see pod metrics that can help identify node launch failures due to bad AMIs. 
- The existing pod binding/ready duration metrics will be maintained alongside these new ones, as they provide valuable data about the overall scheduling timeline

This dual metric system will give us both filtered insights and a complete view of when a scheduling decision was made for the pod and the time it took for it to bound after the decision was made.
