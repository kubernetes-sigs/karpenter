...
The change adds a defaulting step that mirrors the scheduler's `PodTopologySpread` plugin: when the pod's effective constraint list is empty, synthesize `TopologyGroup`s from the config's `podTopologySpread.defaultConstraints` instead. Matching the plugin's semantics:
...
The configured defaults are threaded into `Topology` at construction (`NewTopology`) so `newForTopologies` can reach them without turning its per-pod call into a cluster read. The `SchedulerConfiguration` value is decoded and validated once at startup and carried via context, reaching the scheduler the same way `preference-policy` does today.
...
- **Consolidation.** The default constraints must be applied in the consolidation scheduling simulation as well, not just provisioning — otherwise consolidation could re-pack pods into a distribution the scheduler would reject, re-creating the divergence after the fact. Because the defaults are sourced inside `newForTopologies`, any simulation that builds a `Topology` picks them up automatically.
...
- Extend scheduling-simulation events so that a pod deemed unschedulable *because of a synthesized default constraint* is distinguishable from one unschedulable due to its own spec. This is the primary debugging hazard of cluster-level defaults (use case 3): the constraint is invisible in the pod spec, so without this an operator cannot trace the `Pending` back to its source.
