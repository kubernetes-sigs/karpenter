...
4. **How should a `ScheduleAnyway` default be treated?** This RFC proposes that a synthesized default honor `preference-policy` exactly as a per-pod constraint does, so under the default `Respect` a `ScheduleAnyway` default becomes a hard `TopologyGroup` that is relaxed only after the pod fails to schedule. That is Karpenter's existing approximation of soft spreading, but it diverges from the scheduler in a way that is amplified when the constraint comes from a cluster-wide default rather than an individual workload.

    Upstream splits the two actions across plugin phases: `DoNotSchedule` is enforced in **Filter** (a hard reject), while `ScheduleAnyway` is only ever evaluated in **Score** and can never reject a node. Karpenter has no scoring phase, so under `Respect` it enforces a `ScheduleAnyway` default more strictly than the scheduler does — provisioning a node in an under-represented domain that the real scheduler would have been content to bin-pack past. Three consequences:

    - **Over-provisioning.** Karpenter may launch capacity to satisfy a spread the scheduler treats as a mere preference, for *every* unconstrained workload rather than the few that opted in.
    - **Consolidation pressure.** Karpenter already warns that `pod(s) have a preferred TopologySpreadConstraint which can prevent consolidation`, and suppresses that warning when `preference-policy: Ignore` — the project treats this combination as a known hazard. A cluster-wide default makes it cluster-wide rather than opt-in.
    - **Interaction with question 3.** The scheduler's built-in defaults are *both* `ScheduleAnyway`. If Karpenter ever mirrors them, `Respect` would hard-spread every pod in the cluster while the scheduler merely prefers it.

    Options, roughly in increasing strictness toward the operator:

    1. **Honor `preference-policy` uniformly** (as proposed). Per-pod and defaulted constraints behave identically; the cost is the over-provisioning and consolidation pressure above.
    2. **Always ignore `ScheduleAnyway` defaults**, regardless of `preference-policy` — only `DoNotSchedule` defaults create topology groups. Avoids phantom nodes and the consolidation hazard, and is arguably closer to upstream given that Karpenter cannot score. The costs are a fidelity loss in the other direction (Karpenter may pack where the scheduler would have spread) and defaulted constraints no longer behaving like per-pod ones under the same setting.
    3. **Reject `ScheduleAnyway` in `defaultConstraints` at startup.** Honest about the limitation instead of approximating it, and forward-compatible — soft-spread support could be added later without a behavior change. But it rejects a config `kube-scheduler` accepts, which cuts against the copy-paste goal, and would reject the built-in default set outright.

    Resolving this likely requires deciding question 3 as well, since the built-in defaults are entirely `ScheduleAnyway`.