...
One tells the ReplicaSet controller about scale-down order, the other tells Karpenter about consolidation preference. They need to be split because conflating them on the legacy annotation, as some customers do today, makes understanding the cluster operator's intent more complicated.
...
The deletion cost controller ideally shares the same basic node ranking algorithm as consolidation. While some of this is implementation details, the pod deletion cost controller should call the same function the consolidation controller uses to order candidates. Today that is `sortCandidates` in `pkg/controllers/disruption/consolidation.go`. The Balanced Consolidation work in [#2962](https://github.com/kubernetes-sigs/karpenter/pull/2962) reshapes that sort from disruption-cost ascending to savings-ratio descending. The implementation should lift the comparator out of `sortCandidates` into an exported helper in the same package so both depend on it directly and so that any future updates to candidate sorting are automatically consumed by both controllers.
