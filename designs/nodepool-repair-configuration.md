...
2. **CloudProvider default**: Use the `TolerationDuration` from the CloudProvider's `RepairPolicy`.

Every override is therefore explicit: a condition is only retimed when a policy names it. There is
deliberately no NodePool-wide default, so a CloudProvider adding a new repair condition later cannot
have an unrelated blanket duration silently applied to it.
...
- A `Ready=Unknown` condition has no matching policy, so it keeps the CloudProvider's duration.
