...
Kafka and Cassandra on Kubernetes are typically deployed as StatefulSets backed by persistent EBS volumes. EBS volumes are single-attach: a volume can only be mounted to one EC2 instance at a time. When Karpenter replaces a node, the sequence is strictly serial per volume: evict the pod → detach the EBS volume → provision the new node → attach the volume → start the new pod. There is no way to "pre-warm" a replacement broker alongside the existing one. This means every concurrent node replacement across zones creates a simultaneous partition replica vacancy in each of those zones.
...
**Pod Disruption Budgets** control *how many* pods can be unavailable simultaneously — the eviction pacing dimension. For EBS-backed StatefulSets, pacing is irrelevant to the topology safety problem: the replacement sequence is already serially constrained per volume, and no surge eviction feature can allow a new broker to start before the old one's volume is detached. More fundamentally, PDBs have no topology awareness: there is no way to express "do not evict pods in az-a and az-b at the same time." The problem is about *which* nodes Karpenter selects for simultaneous replacement, not *how* the eviction of each individual pod proceeds.

Custom eviction webhooks can add delays between pod evictions and gate on external signals (e.g. Kafka rebalance-in-progress metrics), which mitigates the severity of rebalance storms. However, they delay the replacement process rather than preventing cross-zone simultaneous disruption. They are a complementary control, not a substitute for topology-aware node selection.
