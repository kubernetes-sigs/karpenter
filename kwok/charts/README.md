...
| settings | object | `{"batchIdleDuration":"1s","batchMaxDuration":"10s","featureGates":{"nodeRepair":false,"podDeletionCostManagement":false,"spotToSpotConsolidation":false},"minValuesPolicy":"Strict","preferencePolicy":"Respect","schedulerConfig":""}` | Global Settings to configure Karpenter |
...
| settings.featureGates | object | `{"nodeRepair":false,"podDeletionCostManagement":false,"spotToSpotConsolidation":false}` | Feature Gate configuration values. Feature Gates will follow the same graduation process and requirements as feature gates in Kubernetes. More information here https://kubernetes.io/docs/reference/command-line-tools-reference/feature-gates/#feature-gates-for-alpha-or-beta-features |
...
| settings.featureGates.podDeletionCostManagement | bool | `false` | podDeletionCostManagement is ALPHA and is disabled by default. Setting this to true will enable Karpenter to manage pod-deletion-cost annotations on pods it schedules, influencing which pod(s) the replica set controller prefers to stop during a replica scale down. |
...
| settings.schedulerConfig | string | `""` | Configures the parts of the cluster's kube-scheduler behavior that Karpenter must mirror during scheduling simulation, currently only podTopologySpread.defaultConstraints. Set this to the same defaultConstraints your kube-scheduler's PodTopologySpread plugin is configured with, so Karpenter accounts for the constraints those pods will actually be scheduled under. As upstream requires, a constraint here must not carry a labelSelector; one is deduced per pod. Empty (the default) means no scheduler-config overrides. |
