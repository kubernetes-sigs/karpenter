	...
	Context("Parse", func() {
		...
		It("shouldn't overwrite CLI flags with environment variables", func() {
			...
			err := opts.Parse(
				...
				"--feature-gates", "CapacityReservations=true,SpotToSpotConsolidation=true,NodeRepair=true",
			)
			Expect(err).To(BeNil())
			expectOptionsMatch(opts, test.Options(test.OptionsFields{
		...
		It("should use environment variables when CLI flags aren't set", func() {
			...
			os.Setenv("FEATURE_GATES", "CapacityReservations=true,SpotToSpotConsolidation=true,NodeRepair=true")
			fs = &options.FlagSet{
				FlagSet: flag.NewFlagSet("karpenter", flag.ContinueOnError),
			}
			opts.AddFlags(fs)
