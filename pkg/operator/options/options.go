...
func (o *Options) AddFlags(fs *FlagSet) {
	...
	fs.StringVar(&o.FeatureGates.inputStr, "feature-gates", env.WithDefaultString("FEATURE_GATES", "NodeRepair=false,ReservedCapacity=false,SpotToSpotConsolidation=false,SyncNodeclaimTaints=true"), "Optional features can be enabled / disabled using feature gates. Current options are: NodeRepair, ReservedCapacity, SpotToSpotConsolidation, and SyncNodeclaimTaints")
}
...
func ParseFeatureGates(gateStr string) (FeatureGates, error) {
	...
	if val, ok := gateMap["SyncNodeclaimTaints"]; ok {
		gates.SyncNodeclaimTaints = val
	}
