...
func (o *Options) AddFlags(fs *FlagSet) {
	...
	fs.DurationVar(&o.NodeReadyTimeout, "node-ready-timeout", env.WithDefaultDuration("NODE_READY_TIMEOUT", 10*time.Minute), "The maximum time to wait for a node to become ready before considering it for recovery")
	...
	fs.StringVar(&o.FeatureGates.inputStr, "feature-gates", env.WithDefaultString("FEATURE_GATES", "NodeRepair=false,ReservedCapacity=false,SpotToSpotConsolidation=false,NodeReadyTimeoutRecovery=false"), "Optional features can be enabled / disabled using feature gates. Current options are: NodeRepair, ReservedCapacity, SpotToSpotConsolidation, and NodeReadyTimeoutRecovery")
}
...
func ParseFeatureGates(gateStr string) (FeatureGates, error) {
	...
	if val, ok := gateMap["NodeReadyTimeoutRecovery"]; ok {
		gates.NodeReadyTimeoutRecovery = val
	}
