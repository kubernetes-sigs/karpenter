...
func (o *Options) AddFlags(fs *FlagSet) {
	...
	fs.BoolVarWithEnv(&o.EagerNodeLimit, "eager-node-limit", "EAGER_NODE_LIMIT", false, "When enabled, node limits are enforced eagerly before NodeClaim creation, preventing the scheduler from exceeding node limits when processing multiple pods in a single batch.")
