...
type FeatureGates struct {
	...
	MultiNodeMultiReplacement bool
	NodeOverlay               bool
	StaticCapacity            bool
	CapacityBuffer            bool
}
...
func (o *Options) AddFlags(fs *FlagSet) {
	...
	fs.StringVar(&o.FeatureGates.inputStr, "feature-gates", env.WithDefaultString("FEATURE_GATES", "NodeRepair=false,ReservedCapacity=true,SpotToSpotConsolidation=false,MultiNodeMultiReplacement=false,NodeOverlay=false,StaticCapacity=false,CapacityBuffer=false"), "Optional features can be enabled / disabled using feature gates. Current options are: NodeRepair, ReservedCapacity, SpotToSpotConsolidation, MultiNodeMultiReplacement, NodeOverlay, StaticCapacity, and CapacityBuffer.")
}
...
func DefaultFeatureGates() FeatureGates {
	return FeatureGates{
		...
		MultiNodeMultiReplacement: false,
		NodeOverlay:               false,
...
func ParseFeatureGates(gateStr string) (FeatureGates, error) {
	...
	if val, ok := gateMap["MultiNodeMultiReplacement"]; ok {
		gates.MultiNodeMultiReplacement = val
	}
