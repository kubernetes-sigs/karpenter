...
type Options struct {
	...
	MinNodeDrainTime        time.Duration
...
func (o *Options) AddFlags(fs *FlagSet) {
	...
	fs.DurationVar(&o.MinNodeDrainTime, "min-node-drain-time", env.WithDefaultDuration("MIN_NODE_DRAIN_TIME", 0*time.Second), "Minimum time to wait after tainting a node before draining pods to ensure the scheduler sees the taint")
