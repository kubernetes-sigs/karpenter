...
func (cc *ClusterCost) internalUpdateOfferings(np *v1.NodePool, instanceTypes []*cloudprovider.InstanceType) {
	...
	if !exists {
		...
		lo.ForEach(instanceTypes, func(it *cloudprovider.InstanceType, _ int) {
			lo.ForEach(it.Offerings, func(o *cloudprovider.Offering, _ int) {
...
func (cc *ClusterCost) internalAddOffering(ctx context.Context, npName string, offeringKey OfferingKey) error {
	...
	if !exists {
		...
		oc, exists = cc.npCostMap[npName].offeringCounts[offeringKey]
		if !exists {
			return fmt.Errorf("failed to find offering %q during retry while searching for instance %q in zone %q with capacity %q in nodepool %q, %w", offeringKey, offeringKey.InstanceName, offeringKey.Zone, offeringKey.CapacityType, npName, err)
		}
	} else {
		oc.Count += 1
	}
