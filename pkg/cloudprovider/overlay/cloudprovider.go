...
func Decorate(cloudProvider cloudprovider.CloudProvider, kubeClient client.Client, store *validation.InstanceTypeStore) cloudprovider.CloudProvider {
...
func (d *decorator) GetInstanceTypes(ctx context.Context, nodePool *v1.NodePool) ([]*cloudprovider.InstanceType, error) {
	its, err := nodepoolutils.PullInstanceTypes(ctx, d.CloudProvider, nodePool)
	if err != nil {
		return []*cloudprovider.InstanceType{}, err
	}
	if options.FromContext(ctx).FeatureGates.NodeOverlay {
		its = d.store.ApplyOverlayOnInstanceTypes(nodePool.Name, its)
	}
	return its, nil
}
