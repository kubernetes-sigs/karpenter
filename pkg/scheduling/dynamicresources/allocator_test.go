...
var _ = Describe("Allocator", func() {
	...
	Describe("Topology requirement narrowing", func() {
		...
		It("should correctly backtrack counters when topology narrows incompatibly", func() {
			// Two devices in different zones, each consuming shared counters.
			// Requesting 2 forces a backtrack after the first device narrows topology.
			// After the failed request, a single-device request should still succeed
			// (proving counters were restored on backtrack).
			inClusterSlices := []dynamicresources.ResourceSlice{
				makeAPISlice("s-counters", "gpu.example.com", "pool-a",
			...
			// Request 2 devices. DFS tries gpu-zone-a first (counters OK, topology narrows to zone-a).
			// Then tries gpu-zone-b for second slot — topology incompatible (zone-b != zone-a).
			// Must backtrack: restore counters for gpu-zone-a, restore requirements/pools.
			claim := makeClaim("c1", exactRequest("req-1", "gpu", 2))
			_, err := alloc.Allocate(ctx, nc, []*resourcev1.ResourceClaim{claim})
			Expect(err).To(HaveOccurred())

			// Verify counter budget was NOT leaked by the backtrack.
			// A single-device request should succeed (counters fully available: 80Gi budget, 40Gi needed).
			nc2 := &fakeNodeClaim{
				id:         unique.Make("nc-2"),
				nodePoolID: unique.Make("test-np"),
				requirements: scheduling.NewRequirements(
	...
	Describe("PerDeviceNodeSelection", func() {
		It("should restrict NodeClaim requirements with zone when device uses NodeSelector", func() {
			inClusterSlices := []dynamicresources.ResourceSlice{
				makeAPISlice("s1", "gpu.example.com", "pool-a",
					withPerDeviceNodeSelection(),
		...
		It("should restrict NodeClaim requirements with label selectors when device uses NodeSelector", func() {
			inClusterSlices := []dynamicresources.ResourceSlice{
				makeAPISlice("s1", "gpu.example.com", "pool-a",
					withPerDeviceNodeSelection(),
					withPerDeviceDevices(resourcev1.Device{
		...
		It("should NOT restrict requirements when device uses AllNodes", func() {
			inClusterSlices := []dynamicresources.ResourceSlice{
				makeAPISlice("s1", "gpu.example.com", "pool-a",
					withPerDeviceNodeSelection(),
					withPerDeviceDevices(resourcev1.Device{Name: "gpu-0", AllNodes: ptr.To(true)}),
					withGeneration(1, 1),
				),
			}
			alloc = dynamicresources.NewAllocator(inClusterSlices, dynamicresources.AllocatedDeviceState{ExclusiveDevices: sets.New[cloudprovider.DeviceID]()}, nil, env.Client)
			...
			Expect(err).ToNot(HaveOccurred())
			Expect(result).ToNot(BeNil())
			Expect(result.Requirements).To(BeEmpty())
		})

		It("should skip device with incompatible per-device NodeSelector and allocate from a compatible device", func() {
			inClusterSlices := []dynamicresources.ResourceSlice{
				makeAPISlice("s1", "gpu.example.com", "pool-a",
					withPerDeviceNodeSelection(),
					withPerDeviceDevices(
		...
		It("should backtrack and restore topology requirements when per-device topology path fails", func() {
			inClusterSlices := []dynamicresources.ResourceSlice{
				makeAPISlice("s1", "gpu.example.com", "pool-a",
					withPerDeviceNodeSelection(),
					withPerDeviceDevices(
			...
			// Request 2 GPUs. DFS picks gpu-0 (zone b), tightens to us-west-2b.
			// gpu-1 (zone a) incompatible, gpu-2 (zone a) incompatible → backtrack gpu-0.
			// Then picks gpu-1 (zone a), tightens to us-west-2a. gpu-2 (zone a) compatible → success.
			claim := makeClaim("c1", exactRequest("req-1", "gpu", 2))
