	...
	for _, volume := range pod.Spec.Volumes {
		...
		if err != nil {
			if errors.IsNotFound(err) {
				log.FromContext(ctx).WithValues("Pod", client.ObjectKeyFromObject(pod), "volume", volume.Name).Error(err, "failed tracking CSI volume limits for volume")
				continue
			}
...
func ResolveDriver(ctx context.Context, kubeClient client.Client, pod *v1.Pod, volumeName string, pvc *v1.PersistentVolumeClaim, storageClassName string) (string, error) {
	...
	if storageClassName == "" {
		log.FromContext(ctx).WithValues("volume", volumeName, "Pod", client.ObjectKeyFromObject(pod), "PersistentVolumeClaim", client.ObjectKeyFromObject(pvc)).V(1).Info("failed tracking CSI volume limits for volume with unbound PVC, no storage class specified")
		return "", nil
	}
	...
	if err != nil {
		...
		if errors.IsNotFound(err) {
			log.FromContext(ctx).WithValues("volume", volumeName, "Pod", client.ObjectKeyFromObject(pod), "PersistentVolumeClaim", client.ObjectKeyFromObject(pvc), "StorageClass", client.ObjectKey{Name: storageClassName}).V(1).Info(fmt.Sprintf("failed tracking CSI volume limits for volume with unbound PVC, %s", err))
			return "", nil
		}
		return "", err
	}
