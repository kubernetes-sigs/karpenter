...
func resolveDriver(ctx context.Context, kubeClient client.Client, pod *v1.Pod, volumeName string, pvc *v1.PersistentVolumeClaim, storageClassName string) (string, error) {
	...
	if storageClassName == "" {
		log.FromContext(ctx).WithValues("pod", pod.Name, "volume", volumeName, "pvc", pvc.Name).Error(nil, "failed tracking CSI volume limits for volume with unbound PVC, no storage class specified")
		return "", nil
	}
	...
	if err != nil {
		...
		if errors.IsNotFound(err) {
			log.FromContext(ctx).WithValues("pod", pod.Name, "volume", volumeName, "pvc", pvc.Name, "storageclass", storageClassName).Error(err, "failed tracking CSI volume limits for volume with unbound PVC, %w")
			return "", nil
		}
		return "", err
	}
