...
// TAKEN FROM STATIC DEPROVISIONING CONTROLLER
// Will need to refactor if RFC is approved.
func (c *Controller) Reconcile(ctx context.Context, np *v1.NodePool) (reconcile.Result, error) {
	ctx = injection.WithControllerName(ctx, "deprovisioning")
	fmt.Println("Reconcile regular deprovisioning controller", np.Name)
	// make it still work on dynmamic nodepools, but don't allow it to run on static nodepools
	if !nodepoolutils.IsManaged(np, c.cloudProvider) || nodepoolutils.IsStatic(np) {
		return reconcile.Result{}, nil
	}
	nodeLimit, ok := np.Spec.Limits[resources.Node]
	if !ok {
		return reconcile.Result{}, nil
	}
	...
	desiredReplicas := nodeLimit.Value()
...
func (c *Controller) Register(_ context.Context, m manager.Manager) error {
	return controllerruntime.NewControllerManagedBy(m).
		...
		// Reconcile on NodePool Create and Update (when limits change and nodepool is not static)
		For(&v1.NodePool{}, builder.WithPredicates(nodepoolutils.IsManagedPredicateFuncs(c.cloudProvider), predicate.NewPredicateFuncs(func(o client.Object) bool {
			fmt.Println("isstatic", nodepoolutils.IsStatic(o.(*v1.NodePool)))
			return !nodepoolutils.IsStatic(o.(*v1.NodePool))
		}),
			predicate.Funcs{
				...
				UpdateFunc: func(e event.UpdateEvent) bool {
					...
					fmt.Println("hasnodepoolnodelimitschanged", HasNodePoolNodeLimitsChanged(oldNP, newNP))
					return HasNodePoolNodeLimitsChanged(oldNP, newNP)
				},
...
func HasNodePoolNodeLimitsChanged(oldNP, newNP *v1.NodePool) bool {
	// Change if presence differs or the limit quantity changed
	oldNodeLimit, oldOK := oldNP.Spec.Limits[resources.Node]
	newNodeLimit, newOK := newNP.Spec.Limits[resources.Node]
	// If one has a node limit and the other doesn't, it's a change
	if oldOK != newOK {
		return true
	}
	// If neither have a node limit, no change
	if !oldOK { // implies !newOK too
		return false
	}
	// Both have limits; compare quantities semantically
	return oldNodeLimit.Cmp(newNodeLimit) != 0
}
