...
var _ = Describe("Static Drift Hash", func() {
	...
	Context("NodeClassRef Defaulting", func() {
		...
		It("should set a cloudprovider default apiversion on a nodeclassref when apiversion is not set", func() {
			...
			ExpectReconcileSucceeded(ctx, nodePoolController, client.ObjectKeyFromObject(nodePool))
			nodePool = ExpectExists(ctx, env.Client, nodePool)
			Expect(nodePool.Spec.Template.Spec.NodeClassRef.APIVersion).To(Equal(cp.NodeClassGroupVersionKind[0].GroupVersion().String()))
		})
		...
		It("should set a cloudprovider default kind on a nodeclassref when kind is not set", func() {
			...
			ExpectReconcileSucceeded(ctx, nodePoolController, client.ObjectKeyFromObject(nodePool))
			nodePool = ExpectExists(ctx, env.Client, nodePool)
			Expect(nodePool.Spec.Template.Spec.NodeClassRef.Kind).To(Equal(cp.NodeClassGroupVersionKind[0].Kind))
		})
		...
		It("should not set cloudprovider default for kind and apiversion if there are more then one supported node class", func() {
			cp.NodeClassGroupVersionKind = []schema.GroupVersionKind{
				{
					Group:   "testgroup.sh",
					Version: "v1test1",
			...
			ExpectReconcileSucceeded(ctx, nodePoolController, client.ObjectKeyFromObject(nodePool))
			nodePool = ExpectExists(ctx, env.Client, nodePool)
			Expect(nodePool.Spec.Template.Spec.NodeClassRef.APIVersion).To(Equal(""))
			Expect(nodePool.Spec.Template.Spec.NodeClassRef.Kind).To(Equal(""))
		})
	})
})
