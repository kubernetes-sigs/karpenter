...
// BenchmarkForEachDomain measures per-pod domain filtering as domain count and taint diversity grow. Each taint
// group is modeled as a NodePool carrying that taint and producing every domain, matching how the domain groups are
// built from real NodePools.
func BenchmarkForEachDomain(b *testing.B) {
	...
	// reject means that pod tolerates no taint; tolerate means that pod tolerates every taint
...
func benchmarkForEachDomain(b *testing.B, domains, taintGroupsPerDomain int, tolerates bool) {
	producers := make([]*topologyNodePool, 0, taintGroupsPerDomain)
	for t := range taintGroupsPerDomain {
		producers = append(producers, &topologyNodePool{
	...
	nodeFilter := MakeTopologyNodeFilter(pod, corev1.NodeInclusionPolicyHonor, corev1.NodeInclusionPolicyIgnore)
