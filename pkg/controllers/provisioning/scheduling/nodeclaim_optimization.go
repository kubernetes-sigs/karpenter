...
// minDaemonOverhead returns the minimum daemon overhead across the NodeClaim's
// daemon overhead groups, mirroring NodeClaim.addDaemonRequests. Unlike
// addDaemonRequests it does not filter by remaining instance types: the
// estimate reasons over the NodePool's original instance type set, so no
// group has been pruned yet.
func (n *NodeClaim) minDaemonOverhead() corev1.ResourceList {
	var minOverhead corev1.ResourceList
	for _, g := range n.daemonOverheadGroups {
		if len(g.DaemonOverhead) == 0 {
			continue
		}
		if len(minOverhead) == 0 {
			minOverhead = g.DaemonOverhead
		} else {
			minOverhead = resources.MinResources(minOverhead, g.DaemonOverhead)
		}
	}
	return minOverhead
}
...
func (n *NodeClaim) estimateCheapestPlacement(pods []*corev1.Pod, podData map[types.UID]*PodData) (*cloudprovider.InstanceType, float64) {
	...
	totalRequests := resources.Merge(n.minDaemonOverhead(), resources.RequestsForPods(pods...))
...
// holdsDRAPod reports whether any pod on the NodeClaim requests devices
// through a ResourceClaim, in which case the optimizer must leave the claim
// alone entirely.
//
// A committed DRA device allocation has no inverse (Allocation.Commit, see
// NodeClaim.Add, is one-way), and RevertTo re-widens InstanceTypeOptions back
// to the snapshot superset. Both make it unsafe for the optimizer to mutate
// any NodeClaim that holds a DRA pod: displacing one would re-queue an
// already-committed allocation, and reverting around a retained one would
// re-advertise instance types the allocator already released for this
// hostname. Skip the whole claim rather than reasoning about split position.
// Keyed on the pod (not on the allocator or IgnoreDRARequests flag) so it
// stays correct after DRA GA removes that flag: non-DRA claims keep
// optimizing, DRA-carrying claims are always left alone.
func (n *NodeClaim) holdsDRAPod(podData map[types.UID]*PodData) bool {
	for _, p := range n.Pods {
		if podData[p.UID].HasResourceClaimRequests {
			return true
		}
	}
	return false
...
// firstIndexBelowPriceThreshold walks the scheduling history backwards from the
// state before current and returns the index of the first state whose price is
// at least minPriceSavingsRatio below currentPrice, or -1 if none is. Walking
// backwards finds the smallest revert that clears the threshold, which
// displaces the fewest pods.
func (n *NodeClaim) firstIndexBelowPriceThreshold(currentPrice float64) int {
	for i := len(n.schedulingOptions) - 2; i >= 0; i-- {
		savings := (currentPrice - n.schedulingOptions[i].Price) / currentPrice
...
func (n *NodeClaim) findSplitPoint(podData map[types.UID]*PodData) int {
	...
	candidateIdx := n.firstIndexBelowPriceThreshold(current.Price)
	if candidateIdx == -1 {
		return -1
	}
...
//	group.HostPortUsage.Add(pod, ...) for each group     -> group.HostPortUsage.DeletePod(key) for each group
//
...
func (n *NodeClaim) releasePodFromSharedState(pod *corev1.Pod, deltas []TopologyDelta) {
	...
	for _, group := range n.daemonOverheadGroups {
		group.HostPortUsage.DeletePod(key)
	}
}
...
//   - Seats dropped mid-sequence are already gone; no re-acquire path, since
...
// offering is also reachable through the snapshot's instance types, so
