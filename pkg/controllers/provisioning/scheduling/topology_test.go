...
var _ = Describe("Topology", func() {
	...
	Context("Zonal", func() {
		...
		It("should only schedule to minimum domains if already violating max skew", func() {
			...
			ExpectProvisioned(ctx, env.Client, cluster, cloudProvider, provisioningController, provisioner, createPods(3)...)
	...
	Context("Combined Zonal Topology and Node Affinity", func() {
		...
		It("should limit spread options by required node affinity", func() {
			...
			ExpectApplied(ctx, env.Client, nodePool)
			ExpectProvisioned(ctx, env.Client, cluster, cloudProvider, provisioningController, provisioner, test.UnschedulablePod(test.PodOptions{
				ObjectMeta:                metav1.ObjectMeta{Labels: labels},
				TopologySpreadConstraints: topology,
				NodeRequirements: []corev1.NodeSelectorRequirement{{Key: corev1.LabelTopologyZone, Operator: corev1.NodeSelectorOpIn, Values: []string{
	...
	Context("Pod Affinity/Anti-Affinity", func() {
		...
		It("should allow two nodes to be created when two pods with matching affinities have incompatible selectors", func() {
			...
			ExpectApplied(ctx, env.Client, nodePool)
			ExpectProvisioned(ctx, env.Client, cluster, cloudProvider, provisioningController, provisioner, pod1, pod2)
			nodeNames := map[string]struct{}{}
		...
		It("should separate nodes using simple pod anti-affinity on hostname", func() {
			...
			for i := 0; i < 10; i++ {
				...
				ExpectProvisioned(ctx, env.Client, cluster, cloudProvider, provisioningController, provisioner, affPod2, affPod1)
				n1 := ExpectScheduled(ctx, env.Client, affPod1)
				n2 := ExpectScheduled(ctx, env.Client, affPod2)
		...
		It("should not violate pod anti-affinity on zone", func() {
			...
			ExpectApplied(ctx, env.Client, nodePool)
			ExpectProvisioned(ctx, env.Client, cluster, cloudProvider, provisioningController, provisioner, zone1Pod, zone2Pod, zone3Pod, affPod)
		...
		It("should not violate pod anti-affinity on zone (other schedules first)", func() {
			...
			ExpectApplied(ctx, env.Client, nodePool)
			ExpectProvisioned(ctx, env.Client, cluster, cloudProvider, provisioningController, provisioner, pod, affPod)
		...
		It("should not violate pod anti-affinity on zone (Schrödinger)", func() {
			...
			ExpectApplied(ctx, env.Client, nodePool)
			ExpectProvisioned(ctx, env.Client, cluster, cloudProvider, provisioningController, provisioner, zoneAnywherePod, affPod)
			...
			ExpectProvisioned(ctx, env.Client, cluster, cloudProvider, provisioningController, provisioner, affPod)
		...
		It("should not violate pod anti-affinity on zone (inverse w/existing nodes)", func() {
			...
			ExpectApplied(ctx, env.Client, nodePool)
			ExpectProvisioned(ctx, env.Client, cluster, cloudProvider, provisioningController, provisioner, zone1Pod, zone2Pod, zone3Pod)
		...
		It("should support pod anti-affinity with a zone topology", func() {
			...
			ExpectApplied(ctx, env.Client, nodePool)
			ExpectProvisioned(ctx, env.Client, cluster, cloudProvider, provisioningController, provisioner, createPods()...)
		...
		It("should not schedule pods with affinity to a non-existent pod", func() {
			...
			ExpectApplied(ctx, env.Client, nodePool)
			ExpectProvisioned(ctx, env.Client, cluster, cloudProvider, provisioningController, provisioner, affPods...)
		...
		It("should support pod affinity with zone topology (unconstrained target)", func() {
			...
			ExpectApplied(ctx, env.Client, nodePool)
			ExpectProvisioned(ctx, env.Client, cluster, cloudProvider, provisioningController, provisioner, append(affPods, targetPod)...)
			top := &corev1.TopologySpreadConstraint{TopologyKey: corev1.LabelTopologyZone}
...
var _ = Describe("Taints", func() {
	...
	It("should schedule pods that tolerate NodePool constraints", func() {
		...
		ExpectProvisioned(ctx, env.Client, cluster, cloudProvider, provisioningController, provisioner, otherPods...)
