...
func (t *Topology) updateInverseAntiAffinity(ctx context.Context, pod *v1.Pod, domains map[string]string) error {
	...
	for _, term := range pod.Spec.Affinity.PodAntiAffinity.RequiredDuringSchedulingIgnoredDuringExecution {
		...
		tg := NewTopologyGroup(TopologyTypePodAntiAffinity, term.TopologyKey, pod, namespaces, term.LabelSelector, math.MaxInt32, nil, t.domains[term.TopologyKey], nil, nil, nil)
...
func (t *Topology) countDomains(ctx context.Context, tg *TopologyGroup) error {
	...
	for i, p := range pods {
		...
		if !tg.nodeFilter.Matches(node, &p, tg) {
			continue
		}
		tg.Record(domain)
	}
...
func (t *Topology) newForTopologies(p *v1.Pod) []*TopologyGroup {
	...
	for _, cs := range p.Spec.TopologySpreadConstraints {
		topologyGroups = append(topologyGroups, NewTopologyGroup(TopologyTypeSpread, cs.TopologyKey, p, sets.New(p.Namespace), cs.LabelSelector, cs.MaxSkew, cs.MinDomains, t.domains[cs.TopologyKey], cs.NodeAffinityPolicy, cs.NodeTaintsPolicy, cs.MatchLabelKeys))
	}
...
func (t *Topology) newForAffinities(ctx context.Context, p *v1.Pod) ([]*TopologyGroup, error) {
	...
	for topologyType, terms := range affinityTerms {
		for _, term := range terms {
			...
			topologyGroups = append(topologyGroups, NewTopologyGroup(topologyType, term.TopologyKey, p, namespaces, term.LabelSelector, math.MaxInt32, nil, t.domains[term.TopologyKey], nil, nil, nil))
		}
	}
	return topologyGroups, nil
}
