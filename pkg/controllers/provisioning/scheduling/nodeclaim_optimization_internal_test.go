...
// cost estimate.
// An optional uidPrefix namespaces the two pods' UIDs so callers can build
// multiple independent claims whose pod-data maps merge without colliding
// (see the per-claim optimizeNodeClaims tests). Omitted for single-claim
// callers, which keep the bare "pod-1"/"pod-2" UIDs.
func setupFindSplitPointClaim(reservedMode ReservedOfferingMode, heldOfferings cloudprovider.Offerings, uidPrefix ...string) (*NodeClaim, map[types.UID]*PodData) {
	prefix := ""
	if len(uidPrefix) > 0 {
		prefix = uidPrefix[0] + "-"
	}
	uid1 := types.UID(prefix + "pod-1")
	uid2 := types.UID(prefix + "pod-2")
	...
	pod1 := &corev1.Pod{ObjectMeta: metav1.ObjectMeta{Name: string(uid1), UID: uid1}}
	pod2 := &corev1.Pod{ObjectMeta: metav1.ObjectMeta{Name: string(uid2), UID: uid2}}

	n := &NodeClaim{
		Pods:                 []*corev1.Pod{pod1, pod2},
		reservedOfferings:    heldOfferings,
		reservedOfferingMode: reservedMode,
		// topology + daemonOverheadGroups + podTopologyDeltas make the claim
		// safe to drive through RevertTo (via optimizeNodeClaims), not just
		// findSplitPoint. No topology mutations are recorded for these pods,
		// so the deltas are nil and Unrecord is a no-op, but the fields must
		// be non-nil so releasePodFromSharedState doesn't panic on a split.
		topology:             &Topology{topologyGroups: map[uint64]*TopologyGroup{}, inverseTopologyGroups: map[uint64]*TopologyGroup{}},
		daemonOverheadGroups: []DaemonOverheadGroup{{HostPortUsage: pscheduling.NewHostPortUsage()}},
		optimizationState: optimizationState{
			...
			podTopologyDeltas: [][]TopologyDelta{nil, nil}, // lockstep with the 2 Pods; no recorded mutations
		},
	}
...
// TestFindSplitPoint_DRAPod_ReturnsMinusOne verifies the DRA guard: a
// NodeClaim holding a pod with resource-claim requests is never split, even
// though the fixture's price/efficiency/net-savings otherwise admit the
// split (the fallback + no-reservation sibling test above proves the same
// fixture splits without a DRA pod). A committed DRA allocation
// (NodeClaim.Add -> Allocation.Commit) has no inverse and RevertTo re-widens
// InstanceTypeOptions, so the optimizer must leave the whole claim alone.
//
// The guard keys on the pod regardless of its position, so it fires whether
// the DRA pod sits in the retained prefix or would be displaced by the split
// This is why the guard skips the whole claim rather than just refusing to
// displace DRA pods.
func TestFindSplitPoint_DRAPod_ReturnsMinusOne(t *testing.T) {
	for _, tc := range []struct {
		name   string
		draUID types.UID
	}{
		{"dra pod in retained prefix", types.UID("pod-1")},
		{"dra pod would be displaced", types.UID("pod-2")},
	} {
		t.Run(tc.name, func(t *testing.T) {
			n, podData := setupFindSplitPointClaim(ReservedOfferingModeFallback, cloudprovider.Offerings{})
			podData[tc.draUID].HasResourceClaimRequests = true

			if got := n.findSplitPoint(podData); got != -1 {
				t.Fatalf("findSplitPoint returned %d; want -1 (a claim holding a DRA pod must never split)", got)
			}
		})
	}
}

// TestFindSplitPoint_DRAGuardIsPerClaim verifies the DRA guard is scoped to
// the individual NodeClaim: marking one claim's pod as DRA suppresses only
// that claim's split, while an identical sibling with no DRA pod still
// splits. optimizeNodeClaims calls findSplitPoint independently per claim, so
// this per-claim behavior is exactly what determines the multi-claim outcome
// A DRA-carrying claim is skipped while its non-DRA siblings in the same
// Solve keep optimizing.
func TestFindSplitPoint_DRAGuardIsPerClaim(t *testing.T) {
	draClaim, draPodData := setupFindSplitPointClaim(ReservedOfferingModeFallback, cloudprovider.Offerings{})
	draPodData[types.UID("pod-1")].HasResourceClaimRequests = true

	plainClaim, plainPodData := setupFindSplitPointClaim(ReservedOfferingModeFallback, cloudprovider.Offerings{})

	if got := draClaim.findSplitPoint(draPodData); got != -1 {
		t.Fatalf("DRA-carrying claim: findSplitPoint returned %d; want -1 (must be skipped)", got)
	}
	if got := plainClaim.findSplitPoint(plainPodData); got < 0 {
		t.Fatalf("non-DRA sibling claim: findSplitPoint returned %d; want a valid split index "+
			"(the DRA guard must not suppress claims without DRA pods). If heuristic thresholds "+
			"were retuned so the fixture no longer splits, update the fixture.", got)
	}
}

// TestOptimizeNodeClaims_DRAClaimNotSplit verifies the guard behaviorally:
// running the actual optimization pass over a NodeClaim that holds a DRA pod
// chooses NOT to split it. Unlike the findSplitPoint tests above (which
// assert the decision), this drives Scheduler.optimizeNodeClaims, the real
// entry point, and asserts the claim is left fully un-mutated: its pods,
// scheduling history, and instance-type options are all intact, and no pods
// were displaced for re-queueing.
func TestOptimizeNodeClaims_DRAClaimNotSplit(t *testing.T) {
	n, podData := setupFindSplitPointClaim(ReservedOfferingModeFallback, cloudprovider.Offerings{})
	podData[types.UID("pod-1")].HasResourceClaimRequests = true

	// Snapshot the pre-pass state the guard must preserve.
	podsBefore := len(n.Pods)
	optionsBefore := len(n.schedulingOptions)
	itsBefore := len(n.InstanceTypeOptions)

	s := &Scheduler{
		newNodeClaims: []*NodeClaim{n},
		cachedPodData: podData,
	}

	q := NewQueue(nil, s.cachedPodData)
	if displaced := s.optimizeNodeClaims(q); displaced {
		t.Fatalf("optimizeNodeClaims displaced pods from a DRA-carrying claim; want no displacement")
	}
	if len(n.Pods) != podsBefore {
		t.Fatalf("DRA claim's Pods changed: %d; want %d (claim must not be split)", len(n.Pods), podsBefore)
	}
	if len(n.schedulingOptions) != optionsBefore {
		t.Fatalf("DRA claim's schedulingOptions truncated: %d; want %d (no revert should have run)", len(n.schedulingOptions), optionsBefore)
	}
	if len(n.InstanceTypeOptions) != itsBefore {
		t.Fatalf("DRA claim's InstanceTypeOptions changed: %d; want %d (no revert should have re-widened them)", len(n.InstanceTypeOptions), itsBefore)
	}
	// The claim is still locked after the pass, exactly as a skipped claim is.
	if !n.locked {
		t.Fatalf("DRA claim not locked after optimizeNodeClaims; want locked (every claim is locked once examined)")
	}
}

// TestOptimizeNodeClaims_DRAClaimSkippedSiblingSplit verifies the guard is
// per-claim in the real pass: with a DRA-carrying claim and an identical
// non-DRA sibling in the same optimizeNodeClaims call, only the sibling
// splits. The DRA claim keeps all its pods; the sibling sheds its
// post-snapshot pod into the re-queue. This is the property that matters on a
// mixed cluster: DRA nodes are left alone while everything else keeps
// getting optimized.
func TestOptimizeNodeClaims_DRAClaimSkippedSiblingSplit(t *testing.T) {
	// Distinct UID prefixes so the two claims' pod-data maps merge without
	// collision; otherwise the sibling's non-DRA "pod-1" would overwrite the
	// DRA claim's flagged "pod-1".
	draClaim, draPodData := setupFindSplitPointClaim(ReservedOfferingModeFallback, cloudprovider.Offerings{}, "dra")
	draPodData[types.UID("dra-pod-1")].HasResourceClaimRequests = true

	sibling, siblingPodData := setupFindSplitPointClaim(ReservedOfferingModeFallback, cloudprovider.Offerings{}, "plain")

	// Merge the two claims' pod data into the single scheduler-level map.
	merged := map[types.UID]*PodData{}
	for k, v := range draPodData {
		merged[k] = v
	}
	for k, v := range siblingPodData {
		merged[k] = v
	}

	s := &Scheduler{
		newNodeClaims: []*NodeClaim{draClaim, sibling},
		cachedPodData: merged,
	}

	q := NewQueue(nil, s.cachedPodData)
	if displaced := s.optimizeNodeClaims(q); !displaced {
		t.Fatalf("optimizeNodeClaims displaced nothing; want the non-DRA sibling to split")
	}

	// DRA claim untouched.
	if len(draClaim.Pods) != 2 {
		t.Fatalf("DRA claim's Pods = %d; want 2 (must not be split)", len(draClaim.Pods))
	}
	if len(draClaim.schedulingOptions) != 2 {
		t.Fatalf("DRA claim's schedulingOptions = %d; want 2 (no revert)", len(draClaim.schedulingOptions))
	}
	// Sibling split at the candidate index: one pod retained, its history
	// truncated to the snapshot.
	if len(sibling.Pods) != 1 {
		t.Fatalf("sibling claim's Pods = %d; want 1 (split should have displaced the second pod)", len(sibling.Pods))
	}
}
...
func TestRevertTo_DeletesHostPortUsageForDisplacedPods(t *testing.T) {
	...
	n := &NodeClaim{
		...
		daemonOverheadGroups: []DaemonOverheadGroup{{HostPortUsage: hpu}},
		optimizationState: optimizationState{
			enabled: true,
			schedulingOptions: []SchedulingOption{
...
func instanceTypeWithReservedOfferings(name, cpu, mem string, ofs ...*cloudprovider.Offering) *cloudprovider.InstanceType {
	offerings := make([]cloudprovider.Offering, len(ofs))
	for i, o := range ofs {
		offerings[i] = *o
	}
	return fake.NewInstanceType(name,
