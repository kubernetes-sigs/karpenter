...
var evictionQueue *ptermination.EvictionQueue
...
var _ = BeforeSuite(func() {
	...
	evictionQueue = ptermination.NewEvictionQueue(ctx, env.KubernetesInterface.CoreV1(), eventRecorder)
	terminationController = termination.NewController(env.Client, ptermination.NewTerminator(fakeClock, env.Client, cloudProvider, evictionQueue), eventRecorder)
})
...
func ExpectNotEnqueuedForEviction(e *ptermination.EvictionQueue, pods ...*v1.Pod) {
	for _, pod := range pods {
