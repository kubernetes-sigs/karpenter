...
var _ = Describe("Candidate Filtering", func() {
	...
	It("should not consider candidates that have do-not-disrupt pods scheduled", func() {
		...
		Expect(cluster.Nodes().Nodes).To(HaveLen(1))
		_, err := disruption.NewCandidate(ctx, env.Client, recorder, fakeClock, cluster.Nodes().Nodes[0], pdbLimits, nodePoolMap, nodePoolInstanceTypeMap, queue)
	...
	It("should consider candidates that have do-not-disrupt terminating pods", func() {
		...
		Expect(cluster.Nodes().Nodes).To(HaveLen(1))
		c, err := disruption.NewCandidate(ctx, env.Client, recorder, fakeClock, cluster.Nodes().Nodes[0], pdbLimits, nodePoolMap, nodePoolInstanceTypeMap, queue)
		Expect(err).ToNot(HaveOccurred())
	...
	It("should not consider candidates that are actively being processed in the queue", func() {
		...
		Expect(cluster.Nodes().Nodes).To(HaveLen(1))
		Expect(queue.Add(orchestration.NewCommand([]string{}, []*state.StateNode{cluster.Nodes().Nodes[0]}, "", "test-method", "fake-type"))).To(Succeed())
