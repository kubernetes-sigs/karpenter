...
var _ = Describe("Candidate Filtering", func() {
	...
	It("should not consider candidates that have do-not-disrupt pods scheduled and no terminationGracePeriod", func() {
		...
		Expect(cluster.Snapshot()).To(HaveLen(1))
		_, err := disruption.NewCandidate(ctx, env.Client, recorder, env.Clock, cluster.Snapshot()[0], pdbLimits, nodePoolMap, nodePoolInstanceTypeMap, queue, disruption.GracefulDisruptionClass)
	...
	It("should consider candidates that have do-not-disrupt pods scheduled with a terminationGracePeriod set for eventual disruption", func() {
		...
		Expect(cluster.Snapshot()).To(HaveLen(1))
		c, err := disruption.NewCandidate(ctx, env.Client, recorder, env.Clock, cluster.Snapshot()[0], pdbLimits, nodePoolMap, nodePoolInstanceTypeMap, queue, disruption.EventualDisruptionClass)
		Expect(err).ToNot(HaveOccurred())
	...
	It("should not consider candidates that have do-not-disrupt pods scheduled without a terminationGracePeriod set for eventual disruption", func() {
		...
		Expect(cluster.Snapshot()).To(HaveLen(1))
		_, err := disruption.NewCandidate(ctx, env.Client, recorder, env.Clock, cluster.Snapshot()[0], pdbLimits, nodePoolMap, nodePoolInstanceTypeMap, queue, disruption.EventualDisruptionClass)
		Expect(err).To(HaveOccurred())
		Expect(err.Error()).To(Equal(fmt.Sprintf(`pod has "karpenter.sh/do-not-disrupt" annotation (Pod=%s)`, client.ObjectKeyFromObject(pod))))
		Expect(recorder.DetectedEvent(fmt.Sprintf(`Pod has "karpenter.sh/do-not-disrupt" annotation (Pod=%s)`, client.ObjectKeyFromObject(pod)))).To(BeTrue())
	})
	...
	It("should consider candidates that have do-not-disrupt terminating pods", func() {
		...
		Expect(cluster.Snapshot()).To(HaveLen(1))
		c, err := disruption.NewCandidate(ctx, env.Client, recorder, env.Clock, cluster.Snapshot()[0], pdbLimits, nodePoolMap, nodePoolInstanceTypeMap, queue, disruption.GracefulDisruptionClass)
		Expect(err).ToNot(HaveOccurred())
		Expect(c.NodeClaim).ToNot(BeNil())
		Expect(c.Node).ToNot(BeNil())
	})
	...
	It("should not consider candidates that are actively being processed in the queue", func() {
		...
		Expect(cluster.Snapshot()).To(HaveLen(1))
		cmd := &disruption.Command{Method: disruption.NewDrift(env.Client, cluster, prov, recorder, env.Clock), Results: pscheduling.Results{}, Candidates: []*disruption.Candidate{{StateNode: cluster.Snapshot()[0], NodePool: nodePool}}, Replacements: nil}
