...
func (c *Controller) MarkDisrupted(ctx context.Context, m Method, candidates ...*Candidate) error {
	...
	err := multierr.Combine(lo.Map(candidates, func(candidate *Candidate, _ int) error {
		// refresh nodeclaim before updating status
		nodeClaim := &v1.NodeClaim{}
		err := c.kubeClient.Get(ctx, client.ObjectKeyFromObject(candidate.NodeClaim), nodeClaim)
		if err != nil {
			return err
		}
		...
		return c.kubeClient.Status().Update(ctx, nodeClaim)
	})...)
	if err != nil {
		c.cluster.UnmarkForDeletion(providerIDs...)
		return fmt.Errorf("updating nodeclaim status: %w", err)
	}
	return nil
}
