...
var _ = Describe("Termination", func() {
	...
	It("should reconstruct a repair termination deadline after the disruption queue restarts", func() {
		repairTerminationGracePeriod := 5 * time.Minute
		nodeClaim.Annotations = map[string]string{
			v1.NodeClaimRepairTerminationGracePeriodAnnotationKey: repairTerminationGracePeriod.String(),
		}
		ExpectApplied(ctx, env.Client, nodePool, nodeClaim)
		ExpectObjectReconciled(ctx, env.Client, nodeClaimController, nodeClaim)
		...
		Expect(nodeClaim.Annotations).To(HaveKeyWithValue(
			...
			nodeClaim.DeletionTimestamp.Add(repairTerminationGracePeriod).Format(time.RFC3339),
		))
		Expect(nodeClaim.Annotations).ToNot(HaveKey(v1.NodeClaimRepairTerminationGracePeriodAnnotationKey))
	})
	DescribeTable("should clear an invalid repair termination intent after the disruption queue restarts",
		func(value string) {
			nodeClaim.Annotations = map[string]string{
				v1.NodeClaimRepairTerminationGracePeriodAnnotationKey: value,
			}
			ExpectApplied(ctx, env.Client, nodePool, nodeClaim)
			ExpectObjectReconciled(ctx, env.Client, nodeClaimController, nodeClaim)
		...
		Entry("when the duration is negative", "-1m"),
		Entry("when the duration is malformed", "invalid"),
	)
	It("should use the NodeClaim termination grace period when the repair intent is invalid", func() {
		terminationGracePeriod := 5 * time.Minute
		nodeClaim.Spec.TerminationGracePeriod = &metav1.Duration{Duration: terminationGracePeriod}
		nodeClaim.Annotations = map[string]string{
			v1.NodeClaimRepairTerminationGracePeriodAnnotationKey: "invalid",
		}
		ExpectApplied(ctx, env.Client, nodePool, nodeClaim)
		ExpectObjectReconciled(ctx, env.Client, nodeClaimController, nodeClaim)
		...
		Expect(nodeClaim.Annotations).To(HaveKeyWithValue(
			v1.NodeClaimTerminationTimestampAnnotationKey,
			nodeClaim.DeletionTimestamp.Add(terminationGracePeriod).Format(time.RFC3339),
		))
		Expect(nodeClaim.Annotations).ToNot(HaveKey(v1.NodeClaimRepairTerminationGracePeriodAnnotationKey))
	})
