...
var _ = Describe("Launch", func() {
	...
	It("should emit InsufficientCapacityError events for the pods mapped to the nodeclaim", func() {
		recorder.Reset()
		cloudProvider.NextCreateErr = cloudprovider.NewInsufficientCapacityError(fmt.Errorf("all instance types were unavailable"))
		nodeClaim := test.NodeClaim()
		pod := test.Pod()
		...
		recorder.ForEachEvent(func(evt events.Event) {
			...
			if p, ok := evt.InvolvedObject.(*corev1.Pod); ok {
				...
				Expect(evt.RateLimiter).To(BeIdenticalTo(nodeclaimlifecycle.InsufficientCapacityErrorPodRateLimiter))
			}
		})
