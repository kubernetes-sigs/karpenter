...
var _ = Describe("NodeClaim/Termination", func() {
	...
	Context("Metrics", func() {
		It("should fire the terminationSummary metric when deleting nodes", func() {
			...
			m, ok := FindMetricWithLabelValues("karpenter_nodes_termination_time_seconds", map[string]string{"nodepool": ""})
			Expect(ok).To(BeTrue())
		...
		It("should fire the nodesTerminated counter metric when deleting nodes", func() {
			...
			m, ok := FindMetricWithLabelValues("karpenter_nodes_terminated", map[string]string{"nodepool": ""})
			Expect(ok).To(BeTrue())
