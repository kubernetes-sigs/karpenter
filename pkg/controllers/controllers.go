	...
	if !options.FromContext(ctx).DisableClusterStateObservability {
		controllers = append(controllers,
			...
			status.NewController[*v1.NodeClaim](
				kubeClient,
				mgr.GetEventRecorderFor("karpenter"), //nolint:staticcheck // TODO: migrate to mgr.GetEventRecorder once operatorpkg status.NewController accepts events.EventRecorder
